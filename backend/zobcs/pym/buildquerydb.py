...
config_profile = zobcs_settings_dict['zobcs_config']
...
def add_cpv_query_pool(mysettings, myportdb, config_id, cp, repo):
	conn =0
	init_package = zobcs_package(mysettings, myportdb)
	# FIXME: remove the check for zobcs when in tree
	if cp != "dev-python/zobcs":
		build_dict = {}
		packageDict = {}
		ebuild_id_list = []
...
def add_buildquery_main(config_id):
	...
	add_zobcs_logs(conn, log_msg, "info", config_profile)
	# Get default config from the configs table  and default_config=1
	default_config_root = "/var/cache/zobcs/" + zobcs_settings_dict['zobcs_gitreponame'] + "/" + config_setup + "/"
	...
	add_zobcs_logs(conn, log_msg, "info", config_is)
